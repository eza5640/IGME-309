...
void MyRigidBody::SetModelMatrix(matrix4 a_m4ModelMatrix)
{
	...
	//your code goes here---------------------
	//first, get the vector positions in local space.
	vector3 minMax[8]; 
	minMax[0] = m_v3MinL;
	minMax[1] = vector3(m_v3MaxL.x, m_v3MinL.y, m_v3MinL.z);
	minMax[2] = vector3(m_v3MinL.x, m_v3MaxL.y, m_v3MinL.z);
	minMax[3] = vector3(m_v3MaxL.x, m_v3MaxL.y, m_v3MinL.z);
	minMax[4] = vector3(m_v3MaxL.x, m_v3MaxL.y, m_v3MaxL.z);
	minMax[5] = vector3(m_v3MaxL.x, m_v3MaxL.y, m_v3MaxL.z);
	minMax[6] = vector3(m_v3MinL.x, m_v3MaxL.y, m_v3MaxL.z);
	minMax[7] = m_v3MaxL;

	//now we convert them to global space
	for (int i = 0; i < 8; i++)
	{
		//cast to a vector 3 (since the array is vector3). use vector 4 because the model matrix is a m4, not a m3.
		minMax[i] = vector3(m_m4ToWorld * vector4(minMax[i], 1.0f));
	}

	//with our points globalized, we need to find the global max and mins
	m_v3MaxG = m_v3MinG = minMax[0];
	for (int i = 0; i < 8; i++)
	{
		if (m_v3MaxG.x < minMax[i].x)
			m_v3MaxG.x = minMax[i].x;
		else if (m_v3MinG.x > minMax[i].x)
			m_v3MinG.x = minMax[i].x;

		if (m_v3MaxG.y < minMax[i].y)
			m_v3MaxG.y = minMax[i].y;
		else if (m_v3MinG.y > minMax[i].y)
			m_v3MinG.y = minMax[i].y;

		if (m_v3MaxG.z < minMax[i].z)
			m_v3MaxG.z = minMax[i].z;
		else if (m_v3MinG.z > minMax[i].z)
			m_v3MinG.z = minMax[i].z;
	}
