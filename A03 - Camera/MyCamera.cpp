...
vector3 MyCamera::GetForward(void)
{
	return m_v3Forward;
}
...
{
	//Normalize forward and then move forwards/backwards.
...
{
	//normalize forward, find right, normalize right, and then find up with forward and right
	glm::normalize(m_v3Forward);
	//up will always be 0, 1, 0.
	vector3 m_v3Right = glm::cross(m_v3Forward, vector3(0.f, 1.f, 0.f));
	glm::normalize(m_v3Right);
	vector3 m_v3Up = glm::cross(m_v3Right, m_v3Forward);
...
{
	//normalize forward and move horizontally
	glm::normalize(m_v3Forward);
	//up will always be 0, 1, 0.
	vector3 m_v3Right = glm::cross(m_v3Forward, vector3(0.f, 1.f, 0.f));
