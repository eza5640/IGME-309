...
void Application::CameraRotation(float a_fSpeed)
{
	...
	matrix3 rotation = matrix3(glm::cos(fAngleY), -glm::sin(fAngleY), 0, glm::sin(fAngleY), glm::cos(fAngleY), 0, 0, 0, 1);
	m_v3Target = rotation * m_v3Target;
	matrix3 rotation2 = matrix3(1, 0, 0, 0, glm::cos(fAngleX), -glm::sin(fAngleX), 0, glm::sin(fAngleX), glm::cos(fAngleX));
	m_v3Target = rotation2 * m_v3Target;
	m_pCamera->SetTarget(m_v3Target);
	m_pCamera->SetForward(glm::normalize(m_v3Target - m_pCamera->GetPosition()));
	SetCursorPos(CenterX, CenterY);//Position the mouse in the center
}
//Keyboard
